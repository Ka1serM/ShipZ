 ...
 * Klasse Spielraster
 *
 * Das Spielfeld besteht aus vier Ebenen (von unten nach oben):
 ...
 * - Rahmen:    Zeigt mit leuchtender Border an, welches Spielfeld gerade aktiv ist.
 *              Nur bei aktivem Zustand werden die Klickevents durchgeleitet.
 *
 * Unter dem Spielfeld befinden sich außerdem:
...
public class Spielraster extends GridPane {
    ...
    // CV
    /** Abstand zwischen dem Spielfeld und den Dekorationen, in Prozent der Breite einer Spielrasterzelle. */
    ...
    /** Zeigt, ob ein Spielfeld aktiv ist und verdeckt sonst die Felder,
    ...
    /** Binding fuer die Offsets zum Spielfeld ohne Dekorationen */
     ...
     */
    public Spielraster(int breite, int hoehe, HashMap<Enums.ShipTyp, Integer> schiffskonfiguration, String spielername, Spielfeld seite) {
        ...
        this.automatischPlatzierenButton.setOnMouseClicked(e -> {
            gui.feuerEvent(Enums.GameAction.SCHIFFE_AUTOMATISCH_PLATZIEREN, "");
        });
        this.add(
            ...
            for(Schuss schuss : zeile) {
                ...
                schuss.setOnMouseClicked(e -> {
                    if (schuss.aktiviert) {
                        ...
                        ShipzGui.getInstance().feuerEvent(Enums.GameAction.SCHUSS_RETURN, schuss.x + "/" + schuss.y);
    ...
    /**
     * Markiert das Spielfeld und lässt Klicks auf Felder durch.
     */
    ...
    /**
     * Entfernt Markierung des Spielfeld und lässt keine Klicks auf Felder mehr durch.
     */
    ...
    /**
     * Setzt einen Schuss in das Spielfeld
    ...
    /**
     * Entfernen eines Schusses aus dem Spielfeld
     * @param x     x-Koordinate
     * @param y     y-Koordinate
     */
    ...
    /**
     * Ändert den Typ eines Schusses im Spielfeld
     * @param x     x-Koordinate
     * @param y     y-Koordinate
     * @param typ   Typ des Schusses
     */
    ...
    public void setzeSchiff(int id, int x, int y, Enums.ShipTyp typ, int rotation, Schiffszustand schiffszustand) {

        // Schiffsobjekt erstellen und in Map speichern
        Schiff schiff = new Schiff(this, id, x, y, typ, rotation,schiffszustand);
        ...
        if (schiffszustand == Schiffszustand.VERSENKT) {
        ...
        if(
            (schiffszustand != Schiffszustand.VERZIEHBAR) &&
            (schiffszustand != Schiffszustand.VERZIEHBAR_FALSCH_POSITIONIERT)
        ) {
     ...
     */
    public void setzeSchiffszustand(int id, Schiffszustand schiffszustand) {
        if(this.schiffe.containsKey(id)) {
            this.schiffe.get(id).setzeSchiffszustand(schiffszustand);
            ...
            ShipzGui.getInstance().debugNachricht("setzeSchiffszustand(): Es wurden vorher keine Schiffe in das Spielfeld geschrieben.", true);
        }
    }
    ...
    public void setzteAlleSchiffeAufUngetroffen() {
        ...
        Set<Map.Entry<Integer,Schiff>> alleSchiffe = this.schiffe.entrySet();
        for(Map.Entry<Integer, Schiff> schiffsEntry : alleSchiffe) {
            schiffsEntry.getValue().setzeSchiffszustand(Schiffszustand.UNGETROFFEN);
