...
public class ShipzGui {
    ...
    //  ---------------------------- IV ----------------------------
        ...
        /** Ansicht zum Waehlen von Spielertyp und Namen eines lokalen Spiels */
        ...
        private HashMap<Enums.ShipTyp, Integer> schiffszusammensetzung;
        ...
        private Spielertyp spielertypLinks;

        /** Typ des Spielers auf der rechten Seite */
        private Spielertyp spielertypRechts;
     ...
     */
    public static ShipzGui createInstance(EventListener einzigerListener, Stage stage) {
     ...
     */
    public void feuerEvent(Enums.GameAction eventTyp, String command) {
        this.einzigerListener.actionPerformed(new Event(this, eventTyp.ordinal(), command));
    }
     ...
     * @return SchiffsID
     */
    public HashMap<Enums.ShipTyp, Integer> getSchiffszusammensetzung() {
     ...
     */
    public void setSchiffszusammensetzung(HashMap<Enums.ShipTyp, Integer> schiffszusammensetzung) {
     ...
     * @param spielernameLinks Name des Spielers am linken Spielfeld
     */
     ...
     * @param spielernameRechts Name des Spielers am linken Spielfeld
     */
     ...
     */
    public Spielertyp getSpielertypLinks() {
        return this.spielertypLinks;
    }

    /**
     * Setzt den Typ des linken Spielers.
     * @param spielertyp Spielertyp
     */
    public void setSpielertypLinks(Spielertyp spielertyp) {
        this.spielertypLinks = spielertyp;
    }
     ...
     */
    public Spielertyp getSpielertypRechts() {
        return this.spielertypRechts;
    }
     ...
     * @param spielertyp Spielertyp
     */
    public void setSpielertypRechts(Spielertyp spielertyp) {
        this.spielertypRechts = spielertyp;
    }
    ...
    private void ansichtenErstellen() {
        ...
        this.ansichten.put(Ansicht.VERBINDE_MIT_HOST, this.ansichtVerbindeMitSpiel = new VerbindeMitSpiel());
        this.ansichten.put(Ansicht.SPIELKONFIGURATION, this.ansichtSpielkonfiguration = new Spielkonfiguration());
        this.ansichten.put(Ansicht.SCHIFFE_ANORDNEN_LINKS, this.ansichtSchiffeAnordnenLinks = new SchiffeAnordnen(Spielfeld.LINKS));
        this.ansichten.put(Ansicht.SCHIFFE_ANORDNEN_RECHTS, this.ansichtSchiffeAnordnenRechts = new SchiffeAnordnen(Spielfeld.RECHTS));
        ...
        this.ansichten.put(Ansicht.WARTE_AUF_CLIENT,this.ansichtWarteAufClient = new WarteAufClient());
        ...
        this.ansichten.put(Ansicht.NETZWERKABBRUCH, this.ansichtNetzwerkverbindungAbgebrochen = new NetzwerkverbindungAbgebrochen());
    ...
    public void spielrundeEinrichten(
            int spiefeldbreite,
            int spielfeldhoehe,
            HashMap<Enums.ShipTyp, Integer> schiffszusammensetzung,
     ...
     * @param spielfeld  Scharfzustellendes Spielfeld
     */
    public void spielfeldKickEventsRegistieren(Spielfeld spielfeld) {

        if(spielraster(spielfeld) != null) {
            spielraster(spielfeld).scharfstellen();
        } else {
    ...
    /**
     * Aktiviert das gewählte Spielfeld.
     * Wenn dieses scharfgestellt ist, kann darauf geschossen werden.
     * Das jeweils andere Spielfeld wird deaktiviert.
     * @param spielfeld  Zu aktivierendes Spielfeld
     */
    public void spielfeldUmrandungSetzen(Spielfeld spielfeld) {
    ...
    /**
     * Setzt einen Schuss in ein Spielfeld
     * @param spielfeld    Linkes oder rechtes Spielfeld
     * @param x            x-Koordinate
     * @param y            y-Koordinate
     * @param typ          Typ des Schusses
     */
    public void setzeSchuss(Spielfeld spielfeld, int x, int y, Schusstyp typ) {

        if(spielraster(spielfeld) != null) {
            spielraster(spielfeld).setzeSchuss(x,y,typ);
        } else {
    ...
    /**
     * Entfernt einen Schuss aus einem Spielfeld
     * @param spielfeld    Linkes oder rechtes Spielfeld
     * @param x            x-Koordinate
     * @param y            y-Koordinate
     */
    public void entferneSchuss(Spielfeld spielfeld, int x, int y) {

        if(spielraster(spielfeld) != null) {
            spielraster(spielfeld).entferneSchuss(x,y);
        } else {
    ...
    /**
     * Ändert einen Schusstyp in einem Spielfeld
     * @param spielfeld    Linkes oder rechtes Spielfeld
     * @param x            x-Koordinate
     * @param y            y-Koordinate
     * @param typ          Typ des Schusses
     */
    public void aendereSchusstyp(Spielfeld spielfeld, int x, int y, Schusstyp typ) {

        if(spielraster(spielfeld) != null) {
            spielraster(spielfeld).aendereSchusstyp(x,y,typ);
        } else {
     ...
     * @param spielfeld         Spielfeld, in welches das Schiff geschrieben werden soll
     * @param id                ID des Schiffes
     * @param x                 x-Koordinate
     * @param y                 y-Koordinate
     * @param typ               Typ des Schiffes
     * @param rotation          Rotation des Schiffes
     * @param schiffszustand    Zustand des Schiffes
     */
    public void setzeSchiff(Spielfeld spielfeld, int id, int x, int y, Enums.ShipTyp typ, int rotation, Schiffszustand schiffszustand) {

        if(spielraster(spielfeld) != null) {
            spielraster(spielfeld).setzeSchiff(id,x,y,typ,rotation,schiffszustand);
        } else {
    ...
    /**
     * Entfernt ein Schiff aus einem Spielfeld
     * @param spielfeld    Linkes oder rechtes Spielfeld
     * @param id           ID des Schiffs
     */
    public void entferneSchiff(Spielfeld spielfeld, int id) {

        if(spielraster(spielfeld) != null) {
            spielraster(spielfeld).entferneSchiff(id);
        } else {
    ...
    /**
     * Entfernt alle Schiff aus einem Spielfeld
     * @param spielfeld    Linkes oder rechtes Spielfeld
     */
    public void entferneAlleSchiffe(Spielfeld spielfeld) {

        if(spielraster(spielfeld) != null) {
            spielraster(spielfeld).entferneAlleSchiffe();
        } else {
     ...
     * @param spielfeld         Linkes oder rechtes Spielfeld
     * @param id                ID des Schiffs
     * @param schiffszustand    Der neue Zustand des Schiffes (UNGETROFFEN, GETROFFEN, VERSENKT, VERZIEHBAR)
     */
    public void setzeSchiffszustand(Spielfeld spielfeld, int id, Schiffszustand schiffszustand) {

        if(spielraster(spielfeld) != null) {
            spielraster(spielfeld).setzeSchiffszustand(id, schiffszustand);
        } else {
     ...
     * @param spielfeld         Linkes oder rechtes Spielfeld
     */
    public void setzeAlleSchiffeAufUngetroffen(Spielfeld spielfeld) {

        if(spielraster(spielfeld) != null) {
            spielraster(spielfeld).setzteAlleSchiffeAufUngetroffen();
        } else {
     ...
     */
    public void aktualisiereScore(Spielfeld spielfeld, int neuerScore) {

        // Score im Spielrasterlabel aktualisieren
        spielraster(spielfeld).aktualisiereScore(neuerScore);
    ...
    /**
     * Gibt je nach Enumwert von Spielfeld das jeweilige Spielraster zurück.
     * @param   spielfeld Spielfeld.LINKS oder Spielfeld.RECHTS
     * @return  Linkes oder rechtes Spielraster
     */
    private Spielraster spielraster(Spielfeld spielfeld) {
        switch (spielfeld) {
            case LINKS:
            ...
            default:
                // Kommt nie vor
                throw new IllegalArgumentException("Ungueltiges Spielfeld");
        }
    }
