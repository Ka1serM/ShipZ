...
public class Schiff extends Pane {
    ...
    public static void schiffsbilderLaden() {
        ...
        for(final ESchiffsTyp schiffsTyp : ESchiffsTyp.values()) {
            for(final ESchiffsZustand ESchiffsZustand : ESchiffsZustand.values()) {
                final String dateiname = (schiffsTyp + "_" + ESchiffsZustand).toLowerCase();
                try {
     ...
     * @param ESchiffsZustand    Optischer Zustand des Schiffs
     * @param spielraster       Spielraster, in welchem sich das Schiff befindet
     */
    public Schiff(final Spielraster spielraster, final int id, final int x, final int y, final ESchiffsTyp typ, final int rotation, final ESchiffsZustand ESchiffsZustand) {
        this.id = id;
        this.typ = typ;
        ...
        this.setzeSchiffszustand(ESchiffsZustand);
        ...
        // Drag & Drop
        if (
            (ESchiffsZustand.VERZIEHBAR == this.schiffszustand) ||
            (ESchiffsZustand.VERZIEHBAR_FALSCH_POSITIONIERT == this.schiffszustand)
        ) {
            ...
            bild.setOnMouseReleased(e -> {
                ...
                if (3 > Math.abs(verschiebungX) && 3 > Math.abs(verschiebungY)) {
                    ...
                    if(passtInsSpielraster(neueRotation,this.x,this.y)) {
                        ...
                        gui.feuerEvent(EAction.SCHIFF_ROTIERT, "");
                    }
                    ...
                    if(passtInsSpielraster(this.rotation, zielkoordinateX, zielkoordinateY)) {
                        ...
                        gui.feuerEvent(EAction.SCHIFF_BEWEGT, "");
     ...
     */
    void setzeSchiffszustand(final ESchiffsZustand ESchiffsZustand) {

        // IV setzen
        this.schiffszustand = ESchiffsZustand;

        // Passendes Bild aus Map laden
        final Image bild = Schiff.schiffsbilder.get((this.typ + "_" + ESchiffsZustand).toLowerCase());
        if (null != bild) {
            ...
            Schiff.gui.debugNachricht("Bild " + (this.typ + "_" + ESchiffsZustand).toLowerCase() + " konnte nicht gefunden werden.",true);
        }

        // Falsche positionierte Schiffe blinken lassen
        if(ESchiffsZustand.VERZIEHBAR_FALSCH_POSITIONIERT == this.schiffszustand) {
...
//    }
    ...
    private boolean passtInsSpielraster(final int rotation, final int x, final int y) {
        ...
        if(0 <= x && 0 <= y) {
            ...
            // Prüfe rechte und untere Kante des ESpielfeld
            switch (rotation) {
                case 0:
                case 180:
     ...
     */
    static SchiffslisteSchiff getSchiffslisteschiff(final ESchiffsTyp typ, final int id) {
    ...
    static class SchiffslisteSchiff extends Pane implements Comparable<SchiffslisteSchiff> {
         ...
         */
        private SchiffslisteSchiff(ESchiffsTyp typ, final int id) {
            this.id = id;
